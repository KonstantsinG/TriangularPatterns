...
var greedy = GreedyTriangulator.new(Vector2(1000, 1000))
...
func test() -> void:
	for am in amounts:
		...
		calculate_time(greedy, "Greedy with " + str(am) + " points: ")


func generate_points(amount : int) -> void:
	for i in range(amount - points.size()):
		points.append(Vector2(randf_range(0, 1000), randf_range(0, 1000)))
