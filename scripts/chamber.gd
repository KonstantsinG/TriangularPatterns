...
## use fullscreen mode for animation area dimensions
## if true - will use window dimensions for bounding_box value
## if false - will use user-defined bounding_box value
@export var fullscreen : bool = true
## draw animation area dimensions
...
@export_group("Points")
## amount of generated points for animation
## 100-300 points give best appearance and performance
@export_range(50, 300, 1, "or_greater") var points_amount : int = 100
## minimum distance between two close points
## lower values will give a more chaotic distribution of points
## higher values will give a more uniform distribution
@export_range(5, 250, 1, "or_greater") var points_min_spacing : float = 50
## instructions on how points will move during the simulation
## static - no movement at all
## directional - points will start moving in a random direction, bouncing off boundary
## chaotic - points will pick a random destination target each frame
@export_enum("Static", "Directional", "Chaotic") var points_movement_mode = 1
## instructions on how points will interact with each other during the simulation
## ignore - points will ignore each other
## repel - points will repel from each other
...
## instructions on how light source will move during the simulation
## static - no movement at all
## circular - light source will circle around bounding box center
## directional - light source will start moving in a random direction, bouncing off outer boundary
## chaotic - light source will pick a random destination target each frame
@export_enum("Static", "Circular", "Directional", "Chaotic") var light_movement_mode = 0
## color gradient for shading triangles
## left color is for the most highlighted tringles
## right color is for the most shaded
@export var light_color_ramp : Gradient
## predefined color gradients
## custom - use color gradient specified in light_color_ramp
...
## triangulation algorithm and triangle grid settings
@export_group("Triangulation")
## priority for triagulation algorithm
## fastest - uses geedy triangulation algorithm for best time and moderate triangle grid quality
## more_accurate - uses delaunay condition and bowyer-watson algorithm for bad time and best triangle grid quality
@export_enum("Fastest", "More accurate") var triangulation_mode = 1
## run resource-intensive operations in separate threads
## if true - triangulation loop will be performed in a separate thread (highly recommended)
## if false - all operations will be performed in the main thread
...
## animation interation settings
@export_group("Interaction")
## use mouse cursor for interations
@export var cursor_interaction : bool = true
## target object to which the interactions will be applied
## points - interactions will affect points
## light - interactions will affect light source
@export_enum("Points", "Light") var interaction_target = 0
## instructions on how points will react on interactions
## attract - mouse cursor (with R/LMB pressed) will attract points
## repel - mouse cursor (with R/LMB pressed) will repel points
## attract_and_repel - mouse cursor with LMB pressed will attract points, with RMB pressed will repel points
@export_enum("Attract", "Repel", "Attract and Repel") var points_interaction_mode = 2
## value that indicates how much interation will affect targets
@export_range(1, 100, 1) var interaction_magnitude : float = 20
## value that indicates how many targets will be affected by interaction
...
## set bounding box according to the given conditions
...
## set light color ramp according to the given conditions
func _set_light_color_ramp() -> void:
	# if color ramp mode is in predefined gradients range - use it
	if color_ramp_mode >= ColorRamp.FIRST_GRADIENT:
		light_color_ramp = ColorRamp.get_gradient_by_id(color_ramp_mode)
	# if animated color ramp mode is selected - create AnimatedGradient
	...
	# if custom color ramp is selected and it is not specified - use default
...
func _generate_points() -> void:
	...
	for i in range(points_amount):
		...
		# and generate random movement direction for it
		if (current_iteration < max_iterations):
			points.push_back(new_point)
			directions.push_back(Vector2(randf() * 2. - 1., randf() * 2. - 1.))
	
	# generate points outside bounding box
	_generate_off_screen_points()


## generate new point inside bounding box
...
## generate points outside bounding box (for animation continuous effect)
func _generate_off_screen_points() -> void:
	...
	directions.push_back(Vector2.ZERO) # zero direction for point means it must be static (not affected by simulation)
	...
	# four off-screen corner points define an off-screen frame
	# divide each segment in that frame into subsegments
	# and place the new point on the boundary of generated segments
...
## generate static points in line segment [a; b]
...
## subdivide line segment [a; b] into several smaller segments
func _subdivide_line_segment(a : Vector2, b : Vector2) -> Array[Vector2]:
	...
	# subdivide segments until length of a subsegment is less than two minimum spacings
	while (a.distance_to(c) > max(50, points_min_spacing * 2)): # using max() to prevent generating too many off-screen points
		for i in range(segments.size()):
			a = segments[i]["from"]
			b = segments[i]["to"]
			c = (a + b) / 2 # subdivide segment by middle point
...
func _triangulation_loop() -> void:
	...
	while true:
		...
		# command for shutdown triangulation thread
...
func _process(delta: float) -> void:
	...
	# animate light color ramp if necessary
...
## check if point intersect simulation boundary
func _check_boundary_intersections(point_idx : int) -> void:
	...
	# if point is crosses boundary, we need to inverse its direction (for auto movement)
	# and fix its position on boundary (for cursor interactions)
...
func _animate_gradient(delta : float) -> void:
	...
	if animated_gradient.interpolation_ratio >= 1.0:
		animated_gradient.next_gradients_pair() 		# take next gradients pair
		animated_gradient.interpolation_ratio = 0.0 	# and reset interpolation ratio
	
	# increase interpolation ratio by frame-dependant step
	animated_gradient.interpolation_ratio += 0.05 * delta
	# update gradient colors by interpolation
...
## draw triangles and triangular grid
func _draw_triangles() -> void:
	...
	for i in range(triangles.size()):
		# calculate distances from triangles center to light source position
	...
	for i in range(triangles.size()):
		# remap distance between min and max found distances
		var remapped_distance = remap(distances_to_light[i], min_distance, max_distance, 0, 1)
		
		# sample triangle color based on remapped distance
		# (use animated or predefined gradient)
...
func _exit_tree() -> void:
	# shutdown triangulation thread before exiting tree
	if use_multiple_threads:
		mutex.lock()
		triangulate = false # send shutdown command
		mutex.unlock()
